...
# one-way flight from airport from_i to airport to_i with cost price_i. 
...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        minStops = {}
        while minHeap:
           cost, destination, stops = heapq.heappop(minHeap)
           if destination == dst:
               return cost
           if stops>k:
               continue
           if destination in minStops and minStops[destination] <= stops:
               continue
           for neighbor,prices in adj[destination]:
               heapq.heappush(minHeap,(cost+prices,neighbor,stops+1))
        return - 1
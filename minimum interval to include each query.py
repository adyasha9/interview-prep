...
# You are given a 2D integer array intervals, where intervals[i] = [left_i, right_i] r
# epresents the ith interval starting at left_i and ending at right_i (inclusive).

# You are also given an integer array of query points queries. The result of query[j] 
# is the length of the shortest interval i such that left_i <= queries[j] <= right_i. 
        ...
        for i in range(len(queries)):
            ...
            for interval in intervals:
                if interval[0]<=queries[i] <=interval[1]:
                    tempQ = interval[1] - interval[0] + 1
                    tempMin = min(tempQ,tempMin)
                    res[i] = tempMin
        return res
